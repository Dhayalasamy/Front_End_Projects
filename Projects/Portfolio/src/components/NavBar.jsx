...
const NavBar = () => {
  ...
  return (
    ...
    <nav className="fixed w-full z-20 top-0 left-0 border-b border-gray-200 Navbar-bg-color ">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div className={ `items-center justify-between w-full md:flex md:w-auto md:order-1 
         ${navTogglebtn ? "block" : "hidden"}` } id="navbar-sticky"  >
          ...
          <ul className={`flex flex-col p-4 md:p-0 mt-4 font-medium border ${navTogglebtn ? "bg-white" : ""} border-gray-100 rounded-lg md:flex-row md:space-x-8 md:mt-0 md:border-0 `}>
            {" "}
            {links.map((link) => {
