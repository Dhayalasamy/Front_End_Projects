...
// these are the axios functions:  formData(),  getState()
export const action =(store, queryClient)=>  async ({ request }) => {
  ...
  try {
    ...
    if (error?.response?.status === 401 || 403) return redirect('/login'); // 401 Unauthorized,  403 Forbidden
    return null;
  }
}
...
const CheckoutForm = () => {
  return (
    ...
    {checkoutFormInputs.map(input=>{
         return <FormInput  key={input.name} label = {input.label} name = {input.name} type = {input.type}/>
        })}
    <div className='mt-4'>
